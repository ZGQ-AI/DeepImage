...
export function setTokens(accessToken: string | null, refreshToken: string | null, mode: StorageMode) {
    setTokenStorageMode(mode);
    const storage = getStorage(mode);
    inMemoryAccessToken = accessToken;
    if (accessToken) storage.setItem(ACCESS_TOKEN_KEY, accessToken); else storage.removeItem(ACCESS_TOKEN_KEY);
    if (refreshToken) storage.setItem(REFRESH_TOKEN_KEY, refreshToken); else storage.removeItem(REFRESH_TOKEN_KEY);
    // 清理另一种存储，避免脏数据
...
export function setAccessToken(token: string | null, mode?: StorageMode) {
    ...
    const storage = getStorage(mode);
    if (token) storage.setItem(ACCESS_TOKEN_KEY, token); else storage.removeItem(ACCESS_TOKEN_KEY);
}
...
export function setRefreshToken(token: string | null, mode?: StorageMode) {
    const storage = getStorage(mode);
    if (token) storage.setItem(REFRESH_TOKEN_KEY, token); else storage.removeItem(REFRESH_TOKEN_KEY);
}
